...
const addItem = (state, action) => {
  ...
  if (existingItemIndex > -1) {
    ...
    const action = { type: 'UPDATE_ITEM', payload: { id, quantity } };
...
const calculateTotal = (items) => {
  ...
  return newItem.reduce((total, item) => total + item.quantity * item.price, 0);
};
...
const calculateTotalItems = (items) => {
  ...
  return newItem.reduce((sum, item) => sum + item.quantity, 0);
};
